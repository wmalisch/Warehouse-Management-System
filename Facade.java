...
public class Facade implements FacadeCommands {
	...
	public void placeOrder(Map<String, Integer> orderDetails, Buyer buyer) {
		...
		Order order = new Order(); // Create order
			order.addOrderItem(new OrderItem(entry.getKey(), entry.getValue()));
		System.out.println("\tPlacing Order");
		controller.depleteStock(order); // Deplete stock
		System.out.println("\tCreating Invoice");
		Invoice invoice = controller.createInvoice(); // Create invoice for order
		bank.receivePayment(invoice, buyer);// Receive payment from client
	...
	public void restock(Map<String, Integer> restockDetails, Supplier supplier) {
		...
		System.out.println("Facade: ");
		Order order = new Order(); // Create order
		for (Map.Entry<String,Integer> entry : restockDetails.entrySet())
			order.addOrderItem(new OrderItem(entry.getKey(), entry.getValue()));
		System.out.println("\tReplenishing Stock");
		controller.replenishStock(order);// Replenish stock according to order
		bank.paySupplier(supplier); // Pay the supplier
	}
